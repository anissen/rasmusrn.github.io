...
private: true
---
In the [last post](/pathfinding.html) I talked about pathfinding. In this post I want to talk about how I integrated pathfinding and other systems into the game.
...
The ultra short introduction to ECS is that everything is modelled as *components* <!-- ... and entities and systems, no? -->. Components are simple data structures that are updated by certain *systems*. For example, if you attach a `GravityComponent` to a game entity the `GravitySystem` will apply gravity to that entity.
...
The above code might seem cumbersome but I think it is worth it. To make the code easier to work with I made a convenience layer called `Database`. Below you see what the example looks like when using the `Database` abstraction:
